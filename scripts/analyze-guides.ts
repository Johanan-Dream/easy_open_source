...
import { buildGuidePrompt, guideDraftSchema, hashReadme, type GuideDraft, type ReviewProposal } from "../src/guide-analysis.ts";
...
for (const repository of repositories) {
  ...
  attempted += 1;
  try {
    const draft = await gemini.generateStructured<GuideDraft>(buildGuidePrompt(repository.id, repository.category, readme, today), guideDraftSchema);
    const candidate: Repository = { ...repository, ...draft, sourceUrls: [repository.githubUrl, draft.guide.officialDocsUrl] };
    ...
    pendingById.set(repository.id.toLowerCase(), {
      ...
      status: "pending", sourceUrls: candidate.sourceUrls, draft,
    });
